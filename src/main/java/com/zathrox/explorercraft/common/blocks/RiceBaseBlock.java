...
public class RiceBaseBlock extends BushBlock implements ILiquidContainer {
    public static final VoxelShape SHAPE = Block.makeCuboidShape(2.0D, 0.0D, 2.0D, 14.0D, 12.0D, 14.0D);


    public RiceBaseBlock(Block.Properties properties) {
        super(properties);
    }

    @Override
    public void onBlockPlacedBy(World worldIn, BlockPos pos, BlockState state, LivingEntity placer, ItemStack stack) {
        worldIn.setBlockState(pos.up(), ExplorerBlocks.RICE_TOP.getDefaultState());
    }
    ...
    public boolean isRiceAboveWater(BlockState state, IBlockReader worldIn, BlockPos pos) {
        return state.getBlock() == ExplorerBlocks.RICE_TOP;
    }
    ...
    public BlockState updatePostPlacement(BlockState stateIn, Direction facing, BlockState facingState, IWorld worldIn, BlockPos currentPos, BlockPos facingPos) {
        ...
        if (facing == Direction.UP && facingState.getBlock() != ExplorerBlocks.RICE_TOP ) {
