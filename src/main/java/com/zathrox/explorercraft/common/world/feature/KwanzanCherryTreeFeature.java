...
public class KwanzanCherryTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {

    private static final BlockState OAK_LOG = ExplorerBlocks.CHERRY_LOG.getDefaultState();
    private static final BlockState OAK_LEAVES = ExplorerBlocks.CHERRY_LEAVES.getDefaultState();

    public KwanzanCherryTreeFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> configFactoryIn, boolean doBlockNotifyIn) {
    ...
    private void makeFoliage(IWorldGenerationReader worldIn, int p_208525_2_, BlockPos pos, List<FoliageCoordinates> p_208525_4_, MutableBoundingBox p_208525_5_, Set<BlockPos> changedBlocks) {
        for(KwanzanCherryTreeFeature.FoliageCoordinates KwanzanCherryTreeFeature$foliagecoordinates : p_208525_4_) {
            if (this.trimBranches(p_208525_2_, KwanzanCherryTreeFeature$foliagecoordinates.getBranchBase() - pos.getY())) {
                this.foliageCluster(worldIn, KwanzanCherryTreeFeature$foliagecoordinates, p_208525_5_, changedBlocks);
            }
        }
    ...
    private void makeBranches(Set<BlockPos> p_208524_1_, IWorldGenerationReader p_208524_2_, int p_208524_3_, BlockPos p_208524_4_, List<KwanzanCherryTreeFeature.FoliageCoordinates> p_208524_5_, MutableBoundingBox p_208524_6_) {
        for(KwanzanCherryTreeFeature.FoliageCoordinates KwanzanCherryTreeFeature$foliagecoordinates : p_208524_5_) {
            int i = KwanzanCherryTreeFeature$foliagecoordinates.getBranchBase();
            BlockPos blockpos = new BlockPos(p_208524_4_.getX(), i, p_208524_4_.getZ());
            if (!blockpos.equals(KwanzanCherryTreeFeature$foliagecoordinates) && this.trimBranches(p_208524_3_, i - p_208524_4_.getY())) {
                this.makeLimb(p_208524_1_, p_208524_2_, blockpos, KwanzanCherryTreeFeature$foliagecoordinates, true, p_208524_6_);
            }
        }
    ...
    public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldIn, Random rand, BlockPos position, MutableBoundingBox boundsIn) {
        ...
        int i = this.checkLocation(changedBlocks, worldIn, position, 10 + random.nextInt(5), boundsIn);
        if (i == -1) {
            return false;
        } else {
            this.setDirtAt(worldIn, position.down(), position);
            int j = (int)((double)i * 0.381D); //Height Attenuation
            if (j >= i) {
                j = i - 1;
            }
            ...
            int i1 = i - 5;
            List<KwanzanCherryTreeFeature.FoliageCoordinates> list = Lists.newArrayList();
            list.add(new KwanzanCherryTreeFeature.FoliageCoordinates(position.up(i1), l));

            for(; i1 >= 0; --i1) {
                float f = this.treeShape(i, i1);
                if (!(f < 0.0F)) {
                    for(int j1 = 0; j1 < k; ++j1) {
                        double d1 = 1.2D; //Scalewidth
                        double d2 = d1 * (double)f * ((double)random.nextFloat() + 0.328D);
                        ...
                        if (this.makeLimb(changedBlocks, worldIn, blockpos, blockpos1, false, boundsIn) == -1) {
                            ...
                            double d6 = (double)blockpos.getY() - Math.sqrt((double)(k1 * k1 + l1 * l1)) * 0.681D; //Branch slope changed
                            ...
                            if (this.makeLimb(changedBlocks, worldIn, blockpos2, blockpos, false, boundsIn) == -1) {
                                list.add(new KwanzanCherryTreeFeature.FoliageCoordinates(blockpos, blockpos2.getY()));
                            }
                        }
                    }
                }
