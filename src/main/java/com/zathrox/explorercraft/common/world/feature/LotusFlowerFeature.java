...
public class LotusFlowerFeature extends Feature<NoFeatureConfig> {
    ...
    private static final BlockState base = ExplorerBlocks.LOTUS_STEM.getDefaultState();

    public LotusFlowerFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> count) {
        super(count);
        block = Blocks.WATER;
    }
    ...
    public boolean place(IWorld world, ChunkGenerator<? extends GenerationSettings> generator, Random rand, BlockPos position, NoFeatureConfig config) {
        ...
        else
        {
            ...
            // Begin lotus flower generation using vanilla loops
                    ...
                    for (int i4 = 0; i4 < 5; ++i4) {
                        ...
                        if (aboolean[(l1 * 16 + i3) * 8 + i4]) {
                            ...
                            if (i4 >= 4) {
                                ...
                                if (world.getBlockState(target) != ExplorerBlocks.LOTUS_FLOWER.getDefaultState() && world.getBlockState(target) != base) {

                                    if (world.getBlockState(target.up()).getBlock() != ExplorerBlocks.BAMBOO_LOG && isSoil == false ) {
            ...
            int lotusFlowerCount = 0;
            int lotusFlowerMax = rand.nextInt(6) + 1;

            while (lotusFlowerCount <= lotusFlowerMax && possibles.size() > 0) {

                BlockPos lotus_flower = possibles.remove(rand.nextInt(possibles.size()));

                if (world.canBlockSeeSky(lotus_flower.up()) && world.isAirBlock(lotus_flower.up())) {

                    BlockState top = ExplorerBlocks.LOTUS_FLOWER.getDefaultState();
                    world.setBlockState(lotus_flower, base, 2);
                    world.setBlockState(lotus_flower.up(), top, 3);
