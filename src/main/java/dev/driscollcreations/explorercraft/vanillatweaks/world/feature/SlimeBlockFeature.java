...
public class SlimeBlockFeature extends Feature<NoneFeatureConfiguration> {
    ...
    protected boolean doPlace(WorldGenLevel world, Random random, double p_207803_4_, double p_207803_6_, double p_207803_8_, double p_207803_10_, double p_207803_12_, double p_207803_14_, int p_207803_16_, int p_207803_17_, int p_207803_18_, int p_207803_19_, int p_207803_20_) {
        int i = 0;
        ...
        for(int k = 0; k < j; ++k) {
            ...
            double d7 = ((double)(Mth.sin((float)Mth.PI * f) + 1.0F) * d6 + 1.0D) / 2.0D;
            adouble[k * 4 + 0] = d0;
            adouble[k * 4 + 1] = d2;
            adouble[k * 4 + 2] = d4;
            adouble[k * 4 + 3] = d7;
        ...
        for(int j3 = 0; j3 < j; ++j3) {
            ...
            if (!(d11 < 0.0D)) {
                ...
                double l = Mth.absMax(Mth.floor(d1 - d11), p_207803_16_);
                double l3 = Mth.absMax(Mth.floor(d3 - d11), p_207803_17_);
                double i1 = Mth.absMax(Mth.floor(d5 - d11), p_207803_18_);
                double j1 = Mth.absMax(Mth.floor(d1 + d11), l);
                double k1 = Mth.absMax(Mth.floor(d3 + d11), l3);
                double l1 = Mth.absMax(Mth.floor(d5 + d11), i1);

                for(int i2 = (int) l; i2 <= j1; ++i2) {
                    double d8 = ((double)i2 + 0.5D - d1) / d11;
                    if (d8 * d8 < 1.0D) {
                        for(int j2 = (int) l3; j2 <= k1; ++j2) {
                            double d9 = ((double)j2 + 0.5D - d3) / d11;
                            if (d8 * d8 + d9 * d9 < 1.0D) {
                                for(int k2 = (int) i1; k2 <= l1; ++k2) {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> pContext) {
        ...
        boolean flag = WorldgenRandom.seedSlimeChunk(chunkpos.x, chunkpos.z, world.getSeed(), 987234911L).nextInt(10) == 0;
        if (random.nextInt(10) == 0 && flag) {
            float f = random.nextFloat() * (float)Mth.PI;
            float f1 = (float)33 / 8.0F;
            int i = Mth.ceil(((float)33 / 16.0F * 2.0F + 1.0F) / 2.0F);
            double d0 = (double)position.getX() + Mth.sin(f) * (double)f1;
            double d1 = (double)position.getX() - Mth.sin(f) * (double)f1;
            double d2 = (double)position.getZ() + Mth.cos(f) * (double)f1;
            double d3 = (double)position.getZ() - Mth.cos(f) * (double)f1;
            int j = 2;
            double d4 = (position.getY() + random.nextInt(3) - 2);
            double d5 = (position.getY() + random.nextInt(3) - 2);
            int k = position.getX() - Mth.ceil(f1) - i;
            int l = position.getY() - 2 - i;
            int i1 = position.getZ() - Mth.ceil(f1) - i;
